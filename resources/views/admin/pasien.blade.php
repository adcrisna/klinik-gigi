                        ...
                        <table class="table table-bordered table-striped" id="data-klinik">
                            ...
                            <tbody>
                                @foreach (@$pasien as $key => $value)
                                    <tr>
                                        <td>{{ @$value->id }}</td>
                                        <td><img src="{{ asset('foto/' . $value->foto) }}" class="zoom" alt="foto pasien">
                                        </td>
                                        <td>{{ @$value->name }}</td>
                                        <td>{{ @$value->nik }}</td>
                                        <td>{{ @$value->alamat }}</td>
                                        ...
                                        <td>
                                            <button class="btn btn-xs btn-success btn-edit-klinik"><i class="fa fa-edit">
                                                    Ubah</i></button> &nbsp;
                                            <a href="{{ route('admin.deletePasien', $value->id) }}"><button
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Form Tambah Data Pasien</h4>
                </div>
                <div class="modal-body">
                    <form action="{{ route('admin.addPasien') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group has-feedback">
                            ...
                            <input type="text" name="nik" class="form-control" placeholder="NIK Pasien" required>
                        </div>
                        <div class="form-group has-feedback">
                            <label>Alamat</label>
    ...
    <div class="modal fade" id="modal-form-edit-klinik" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Form Ubah Data Pasien</h4>
                </div>
                <div class="modal-body">
                    <form action="{{ route('admin.updatePasien') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group has-feedback">
                        ...
                        <div class="form-group has-feedback">
                            <label>Nama Pasien</label>
                            <input type="text" name="name" class="form-control" placeholder="Nama Pasien"
                                required>
                        </div>
                        <div class="form-group has-feedback">
                            <label>NIK Pasien</label>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#data-klinik').on('click', '.btn-edit-klinik', function() {
            ...
            $('input[name=nik]').val(row[3]);
            $('textarea[name=alamat]').val(row[4]);
            $('input[name=no_hp]').val(row[5]);
            $('input[name=email]').val(row[6]);
            $('input[name=tanggal_lahir]').val(row[7]);
            $('select[name=jenis_kelamin]').val(row[8]);
