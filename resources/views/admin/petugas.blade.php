                        ...
                        <table class="table table-bordered table-striped" id="data-klinik">
                            <thead>
                                <tr>
                                    ...
                                    <th>No Telepon</th>
                                    <th>Email</th>
                                    <th style="display: none">Klinik ID</th>
                                    <th>Klinik</th>
                                    <th>Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach (@$petugas as $key => $value)
                                    <tr>
                                        <td>{{ @$value->id }}</td>
                                        <td><img src="{{ asset('foto/' . $value->foto) }}" class="zoom" alt="foto dokter">
                                        </td>
                                        <td>{{ @$value->name }}</td>
                                        <td>{{ @$value->alamat }}</td>
                                        <td>{{ @$value->no_hp }}</td>
                                        <td>{{ @$value->email }}</td>
                                        <td style="display: none">{{ @$value->klinik_id }}</td>
                                        <td>{{ @$value->Klinik->name }}</td>
                                        <td>
                                            <button class="btn btn-xs btn-success btn-edit-klinik"><i class="fa fa-edit">
                                                    Ubah</i></button> &nbsp;
                                            <a href="{{ route('admin.deleteDokter', $value->id) }}"><button
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Form Tambah Data Petugas</h4>
                </div>
                <div class="modal-body">
                    <form action="{{ route('admin.addPetugas') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
    ...
    <div class="modal fade" id="modal-form-edit-klinik" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Form Ubah Data Petugas</h4>
                </div>
                <div class="modal-body">
                    <form action="{{ route('admin.updatePetugas') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group has-feedback">
                        ...
                        <div class="form-group has-feedback">
                            <label>Nama Petugas</label>
                            <input type="text" name="name" class="form-control" placeholder="Nama Petugas"
                                required>
                        </div>
                        <div class="form-group has-feedback">
                            <label>Alamat</label>
